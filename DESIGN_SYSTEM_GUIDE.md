...
var(--accent-red-dark)   /* #bd284d - Use for button hover states */
var(--accent-red-light)  /* #e99d96 - Use for subtle accents */

var(--primary)           /* #d9e2cf - Use for backgrounds, neutral surfaces */
var(--primary-dark)      /* #afbea7 - Use for subtle borders, shadows */
var(--primary-light)     /* #e7e9e2 - Use for light backgrounds */

var(--secondary)         /* #4c99e6 - Use for card gradients, sky elements */
var(--secondary-dark)    /* #2e5cb8 - Use for darker accents */
var(--secondary-light)   /* #99cce6 - Use for card backgrounds */

/* System Colors */
var(--background)        /* Main background color */
var(--foreground)        /* Main text color (#1a1a1a) */
var(--card)             /* Card background */
var(--border)           /* Border color (#1a1a1a) */
```
...
## 🎴 Card System (Already Implemented)
...
## 🛠️ Utility Classes (Already Implemented)
...
<div className="grid grid-cols-2 gap-4">
  <div className="retro-card p-6 cursor-pointer hover:scale-[1.02] transition-all">
    <Book className="h-6 w-6 text-[var(--foreground)]" />
...
1. **Use Component Variants**: Always use Button component variants instead of custom styling
2. **Use CSS Variables**: Reference `var(--accent-red)` instead of hardcoded colors
3. **Use Utility Classes**: Apply `.retro-card`, `.animate-retro-pulse`, etc. for consistent styling
4. **Semantic HTML**: Use proper HTML tags (`h1`, `h2`, `p`) for automatic styling
5. **Accessibility First**: All components include proper ARIA labels and keyboard navigation
...
| Game                              | Study For                                        |
| --------------------------------- | ------------------------------------------------ |
| **Pokémon FireRed/LeafGreen**     | Classic tile logic, clean UI, bold palettes      |
| **Pokémon Mystery Dungeon (GBA)** | Character portraits, dungeons, dramatic lighting |
| **Stardew Valley**                | Modern UI cohesion with retro assets             |
| **Chrono Trigger**                | Spell animations, dynamic light effects          |
| **EarthBound**                    | Emotional tone through minimalist assets         |
